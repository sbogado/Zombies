...
public class ZombiesScene extends GameScene implements TimeSubject{
	...
	private SoundBuilderZombie nuevaOleadaSound = new SoundBuilderZombie(this.getClass().getClassLoader().getResourceAsStream("nueva oleada.wav"));
	...
	public ZombiesScene(Dimension dimension,int cantidadDePixelesPorVertice,ReproductorDeSonidoDeFondo sonido,Zombies game){
		super();
		this.setGame(game);
	...
	private void setObserversToMonsters() {
		...
		monstruos.addAll(this.pool.getFastZombies());
		monstruos.addAll(this.pool.getFatZombies());
		monstruos.addAll(this.pool.getSlowZombies());
		
		for(MonsterObserver observer : getGame().getMonsterObservers()){
			setObserverToMonster(monstruos,observer);
		}
	}

	private void setObserverToMonster(List<Monstruo> monstruos, MonsterObserver observer) {
		
		for(Monstruo monstruo : monstruos){
			monstruo.getObservers().add(observer);
		}
	}

	public void checkEnd(){
		if(getGame() != null && getGame().getMission().isMissionAcomplished()){
			winGame();
		}
	}
	...
	public void cargarMiraDePersonaje() {
		...
		Sprite[] spritesMira  = {Sprite.fromImage("mira1.png"),Sprite.fromImage("mira2.png"),Sprite.fromImage("mira3.png")};
...
//		this.oleadas.add(new Oleada(this,1,1,0.5,80));
	...
	public void cargarPersonaje() {
		...
		SpriteMoved[] sprites2 = {SpriteMoved.fromImage("personajeQuieto.png"),SpriteMoved.fromImage("personajeQuieto.png"),SpriteMoved.fromImage("personajeQuieto.png")};
		AnimationRotateMoved personajeParado = new AnimationRotateMoved(.01,sprites2);
		 Personaje personaje = new Personaje(((Zombies)getGame()).getPlayer(), personajeParado,personajeX,personajeY, 0, dimension.getWidth(), 0  , dimension.getHeight(),
	...
	public void cargarPuntajeJugador() {
		...
		PuntajeJugador puntajeJugador = new PuntajeJugador(
				(dimension.getWidth()/2) - 250 , (dimension.getWidth()/2)-400);
	...
	public void cargarPuntajeMonstruosMuertos() {
		...
		getGame().getMonsterObservers().add(puntajeMonstruosMuertos);
...
//		SpriteMoved[] sprites  = {SpriteMoved.fromImage("slowZombie.png"),SpriteMoved.fromImage("slowZombie.png"),SpriteMoved.fromImage("slowZombie.png")};
//		AnimationRotateMoved animation = new AnimationRotateMoved(.01,sprites);
//		Monstruo monstruo = new SlowZombie(animation,new Vector2D(0,-1),50, 50, this);
//		this.monstruos.add(monstruo);
//		this.addComponent(monstruo);
//	}
