...
import zombie.zombiescene.algoritmosdemonstruos.Shadow;
...
public abstract class Monstruo extends GameComponent<ZombiesScene>{
	...
	public Monstruo(  double xInicial, double yInicial,double puntoAtaqueX,double puntoAtaqueY, Mapa mapa, int velocidad, int puntos, double recuperacionDeImpacto, int danio, int vida,double tiempoDeGolpe,ZombiesScene scene) {
...
//		this.algoritmoDeBusqueda = new Dijkstra(( (108*71)+1),10000, mapa);
	...
	public Vector2D nuevaPosicion(DeltaState deltaState){
		return (this.getDireccion().asVersor().producto(this.getVelocidad()*deltaState.getDelta()).suma(new Vector2D(this.getX(), this.getY())));
	}
	
	
	public void obtenerCaminoHaciaPersonaje(Personaje personaje){
		this.setCaminoHaciaPersonaje(this.getAlgoritmoDeBusqueda().getCamino((int)this.getX(),(int)this.getY(),(int) personaje.getX(),(int) personaje.getY()));
	}
	...
	public void moverAlaSiguientePosicionSimple(int x,int y, DeltaState delta){
		...
		while(!((this.getX() >= x-25 && this.getX() <= x + 25) && (this.getY() >= y-25  && this.getY() <= y +25))){
...
//		double rotation = Zombies.calculateRotation(this.getX(),this.getY(),barricada.getX()+barricada.getWidth()/2,barricada.getY()+barricada.getHeight()/2);
...
//		this.label.render(this, graphics);
	...
	public void setTiempoDeRecuperacionDeImpacto(
			double tiempoDeRecuperacionDeImpacto) {
